      ...
      pkgs.writeShellScriptBin "surrealdb-setup" ''
        ...
        ${lib.optionalString (cfg.initialDatabases != [ ]) ''
          ...
          ${lib.concatMapStringsSep "\n" (db: ''
            ${cfg.package}/bin/surreal sql --log ${cfg.logLevel} --conn http://${cfg.bind}:${toString cfg.port} --user ${cfg.user} --pass ${cfg.password} --query "DEFINE DATABASE IF NOT EXISTS ${db};" || {
        ...
        ${lib.optionalString (cfg.initialScript != null) ''
          ...
          echo ${lib.escapeShellArg cfg.initialScript} | ${cfg.package}/bin/surreal sql --log ${cfg.logLevel} --conn http://${cfg.bind}:${toString cfg.port} --user ${cfg.user} --pass ${cfg.password} || {
